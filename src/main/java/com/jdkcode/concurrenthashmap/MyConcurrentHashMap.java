...
public class MyConcurrentHashMap<K,V> extends AbstractMap<K,V>
    ...
    /**
     * 默认的并发强度，没有用但是定义出来为了兼容以前的版本
     * */
    private static final int DEFAULT_CONCURRENCY_LEVEL = 16;


    /**
     * 加载因子，重载的时候修改这个值的话，只能够影响初始化表的大小，扩展的时候一般使用
     * {@code n-(n >>> 2)}
     * */
    ...
    // onlyIfAbsent默认为：false
    final V putVal(K key, V value, boolean onlyIfAbsent) {
        if (key == null || value == null) throw new NullPointerException();
        //根据key值hashCode，得到hash值
        int hash = spread(key.hashCode());
        int binCount = 0;
        //配合tabAt，casTabAt 的无限循环
        for (Node<K,V>[] tab = table;;) {//for开始
            Node<K,V> f; int n, i, fh;
            if (tab == null || (n = tab.length) == 0)
                //不在新建的时候，初始化Table，选择在使用的时候，初始化
                tab = initTable();
            else if ((f = tabAt(tab, i = (n - 1) & hash)) == null) {
              // tabAt 这个是检查hash对应的位置是否为Null，如果为Null，直接的
              //casTabAt 直接的设置node节点
                if (casTabAt(tab, i, null,
                             new Node<K,V>(hash, key, value, null)))
                    break;                   // no lock when adding to empty bin
            }
            else if ((fh = f.hash) == MOVED)
              // 如果不为null，说明这个位置已经存在了Node节点
              // 然后去判断node节点的hash的值是否为 MOVED, hash的值为final类型
              // MOVED：hash for forwarding nodes（forwarding nodes的hash值）、标示位
              // 检查table[i]的节点的hash是否等于MOVED，如果等于，则检测到正在扩容，
              // 则帮助其扩容
                tab = helpTransfer(tab, f);
            else {
                V oldVal = null;
                // synchronized 加锁的是首节点
                synchronized (f) {
                  //首节点的校验
                    if (tabAt(tab, i) == f) {
                        if (fh >= 0) {
                        ...
                        }else if (f instanceof TreeBin) { // 从这句判断TreeBin 节点的hash值小于零
                            Node<K,V> p;
                            binCount = 2;
                            //树形结构的插入操作
    ...
    private final Node<K,V>[] initTable() {
        ...
        while ((tab = table) == null || tab.length == 0) {
            ...
            if ((sc = sizeCtl) < 0) {
              //thread.yield 可以解释为线程退让，就是让出CPU
              Thread.yield(); // lost initialization race; just spin
            }
            // 如果sizeCtl 大于0，则去抢占扩容的操作：CAS的比较设置循环验证，设置SIZECTL为-1
    ...
    abstract static class CollectionView<K,V,E>
         ...
         */
        public MyConcurrentHashMap<K,V> getMap() { return map; }
    ...
    static final class ValuesView<K,V> extends CollectionView<K,V,V>
        ...
        private static final long serialVersionUID = 2249069246763182397L;
        ValuesView(MyConcurrentHashMap<K,V> map) { super(map); }
    ...
    static final class EntrySetView<K,V> extends CollectionView<K,V,Map.Entry<K,V>>
        ...
        private static final long serialVersionUID = 2249069246763182397L;
        EntrySetView(MyConcurrentHashMap<K,V> map) { super(map); }
